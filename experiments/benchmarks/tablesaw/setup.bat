...
COPY "%POM%" "%DEPCLEAN_POM%"

powershell -Command "& {$ignoredDependencies='it.unimi.dsi:fastutil:8.5.13:compile,io.github.classgraph:classgraph:4.8.168:compile,com.univocity:univocity-parsers:2.9.1:compile,org.slf4j:slf4j-api:2.0.12:compile'; $pomFile='!DEPCLEAN_POM!'; $xml=[xml](Get-Content $pomFile); $ns=$xml.DocumentElement.NamespaceURI; if(-not $xml.project.build){$build=$xml.CreateElement('build', $ns);$xml.project.AppendChild($build)|Out-Null}; if(-not $xml.project.build.plugins){$plugins=$xml.CreateElement('plugins', $ns);$xml.project.build.AppendChild($plugins)|Out-Null}; $plugin=$xml.CreateElement('plugin', $ns); $groupId=$xml.CreateElement('groupId', $ns);$groupId.InnerText='se.kth.castor'; $artifactId=$xml.CreateElement('artifactId', $ns);$artifactId.InnerText='depclean-maven-plugin'; $version=$xml.CreateElement('version', $ns);$version.InnerText='2.0.6'; $execution=$xml.CreateElement('execution', $ns); $goals=$xml.CreateElement('goals', $ns); $goal=$xml.CreateElement('goal', $ns);$goal.InnerText='depclean'; $goals.AppendChild($goal)|Out-Null; $configuration=$xml.CreateElement('configuration', $ns); $ignoreDependenciesTag=$xml.CreateElement('ignoreDependencies', $ns); $ignoreDependenciesTag.InnerText=$ignoredDependencies; $createPomDebloatedTag=$xml.CreateElement('createPomDebloated', $ns); $createPomDebloatedTag.InnerText='true'; $configuration.AppendChild($ignoreDependenciesTag)|Out-Null; $configuration.AppendChild($createPomDebloatedTag)|Out-Null; $execution.AppendChild($goals)|Out-Null; $execution.AppendChild($configuration)|Out-Null; $executions=$xml.CreateElement('executions', $ns); $executions.AppendChild($execution)|Out-Null; $plugin.AppendChild($groupId)|Out-Null; $plugin.AppendChild($artifactId)|Out-Null; $plugin.AppendChild($version)|Out-Null; $plugin.AppendChild($executions)|Out-Null; $xml.project.build.plugins.AppendChild($plugin)|Out-Null; $xml.Save($pomFile)}"

echo 2. Running depclean with modified pom-depclean.xml
call mvn -f "%DEPCLEAN_POM%" clean install

echo 2. Renaming pom-debloated.xml to pom-depclean-debloated.xml
REN "%PROJECT_DIR%\pom-debloated.xml" pom-depclean-debloated.xml

echo 3. Running pom-depclean.xml to build artifact
call mvn -f "%PROJECT_DIR%\pom-depclean-debloated.xml" clean install
...
powershell -Command "& {$ignoredDependencies='it.unimi.dsi:fastutil:8.5.13:compile,tech.tablesaw:tablesaw-core:0.44.1:compile'; $pomFile='!DEPTRIM_POM!'; $xml=[xml](Get-Content $pomFile); $ns=$xml.DocumentElement.NamespaceURI; if(-not $xml.project.build){$build=$xml.CreateElement('build', $ns);$xml.project.AppendChild($build)|Out-Null}; if(-not $xml.project.build.plugins){$plugins=$xml.CreateElement('plugins', $ns);$xml.project.build.AppendChild($plugins)|Out-Null}; $plugin=$xml.CreateElement('plugin', $ns); $groupId=$xml.CreateElement('groupId', $ns);$groupId.InnerText='se.kth.castor'; $artifactId=$xml.CreateElement('artifactId', $ns);$artifactId.InnerText='deptrim-maven-plugin'; $version=$xml.CreateElement('version', $ns);$version.InnerText='0.1.2'; $execution=$xml.CreateElement('execution', $ns); $goals=$xml.CreateElement('goals', $ns); $goal=$xml.CreateElement('goal', $ns);$goal.InnerText='deptrim'; $goals.AppendChild($goal)|Out-Null; $configuration=$xml.CreateElement('configuration', $ns); $singlePom=$xml.CreateElement('createSinglePomSpecialized', $ns); $singlePom.InnerText='true'; $ignoreDependenciesTag=$xml.CreateElement('ignoreDependencies', $ns); $ignoreDependenciesTag.InnerText=$ignoredDependencies; $configuration.AppendChild($singlePom)|Out-Null; $configuration.AppendChild($ignoreDependenciesTag)|Out-Null; $execution.AppendChild($goals)|Out-Null; $execution.AppendChild($configuration)|Out-Null; $executions=$xml.CreateElement('executions', $ns); $executions.AppendChild($execution)|Out-Null; $plugin.AppendChild($groupId)|Out-Null; $plugin.AppendChild($artifactId)|Out-Null; $plugin.AppendChild($version)|Out-Null; $plugin.AppendChild($executions)|Out-Null; $xml.project.build.plugins.AppendChild($plugin)|Out-Null; $xml.Save($pomFile)}"
